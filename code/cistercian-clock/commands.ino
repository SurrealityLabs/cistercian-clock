...
const commandList_t masterCommands[] = {
  ...
  {"stripTest",             stripTestHandler,           "stripTest"},
...
bool stripTestHandler(Commander &Cmdr) {
  Cmdr.println("Testing red...");
  render_fillRed();
  delay(500);
  Cmdr.println("Testing green...");
  render_fillGreen();
  delay(500);
  Cmdr.println("Testing blue...");
  render_fillBlue();
  delay(500);
  render_clear();
  Cmdr.println("Done");
...
bool printLastRebootHandler(Commander &Cmdr) {
  ...
  tmp_t = myTZ.toLocal(lastRebootTime);
  breakTime(tmp_t, newTime);
  snprintf(strbuf, 32, "%04d-%02d-%02d %02d:%02d:%02d ", tmYearToCalendar(newTime.Year), newTime.Month, newTime.Day, newTime.Hour, newTime.Minute, newTime.Second);
  Cmdr.println(strbuf);
  return 0;  
